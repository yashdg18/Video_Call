...
const login = async (req, res) => {
    ...
    try {
        ...
        if (!isPasswordCorrect) return res.status(httpStatus.UNAUTHORIZED).json({ message: "Invalid credentials" });
...
const addToHistory = async (req, res) => {
    ...
    if (!token || !meeting_code) return res.status(httpStatus.BAD_REQUEST).json({ message: "Token and meeting_code are required" });
    try {
        const user = await User.findOne({ token });
        if (!user) return res.status(httpStatus.UNAUTHORIZED).json({ message: "Invalid token" });
